...
describe('AuthService', () => {
  ...
  it('can create a new user with salted and hashed password', async () => {
    const user = await service.signup('asdfasdf@asdf.com', 'asdf');
  ...
  it('throws an error if user signs up with email that is in use', async () => {
    fakeUsersService.find = () =>
      Promise.resolve([{ id: 1, email: 'a', password: 'b' } as UserEntity]);
    await expect(service.signup('asdfasdf@asdf.com', 'asdf')).rejects.toThrow(
      BadRequestException,
    );
  });
